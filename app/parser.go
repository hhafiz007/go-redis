...
func parseBulkString(stream *bufio.Reader) (RedisMessage, error) {
	...
	length, _ := strconv.Atoi(string(bytes[0 : len(bytes)-2]))
	data := make([]byte, length+2)
	fmt.Println("The length of data is ", length, bytes)

	_, err = stream.Read(data)

	if err != nil {
		fmt.Println("Error reaadaainag bulk string")
		return RedisMessage{}, err
	}
	...
	fmt.Println("Bulak Striang is", myMsg, string(myMsg.bytes))
